...
export async function POST({ request, cookies }) {
  try {
    ...
    // Check for either admin access OR apikey.delete permission
    const { userHasPermission } = await import("$lib/userProfile.js");
    const hasDeletePermission =
      isAdmin || (await userHasPermission(localUserId, "apikey.delete"));
      ...
      return json(
        {
          success: false,
          error:
            "Insufficient permissions. You need the 'apikey.delete' permission to delete API keys.",
        },
        { status: 403 },
      );
    }
    ...
    // Delete the API key
    const success = await deleteApiKey(key_id, localUserId);

    if (!success) {
      return json(
        { success: false, error: "Failed to delete API key or key not found" },
        { status: 404 },
      );
    }
