...
import {
  ROMM_SERVER_URL as ROMM_SERVER_URL_RAW,
  ROMM_USERNAME,
  ROMM_PASSWORD,
} from "$env/static/private";
import { getGameById } from "./gameCache.js";

// Environment variables for ROMM configuration - no fallbacks
const ROMM_SERVER_URL = ROMM_SERVER_URL_RAW;
...
async function authenticateROMM() {
  if (!ROMM_SERVER_URL || !ROMM_USERNAME || !ROMM_PASSWORD) {
    console.warn("⚠️ ROMM server URL or credentials not configured - ROMM features disabled");
    return null;
  }
 ...
 */
export function isRommConfigured() {
...
export async function crossReferenceWithROMM(igdbGames, cookies = null) {
  if (!isRommConfigured() || !(await isRommAvailable(cookies))) {
    return igdbGames;
  }
