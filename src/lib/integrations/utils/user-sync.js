...
export function normalizeUserData(userData, provider) {
  ...
  const normalized = {
    external_id: userData.id || userData.user_id || userData.external_id || userData.sub,
    email: userData.email,
    name: userData.name || userData.display_name || userData.preferred_username || userData.username,
  ...
  try {
    ...
    if (existingUser.rows.length > 0) {
      ...
      result = await query(`
        ...
        RETURNING *
      `, [
        userId, payload.external_id, payload.email, payload.name,
        payload.avatar, payload.is_active, payload.last_synced_at,
      ...
      result = await query(`
        ...
        RETURNING *
      `, [
        newUserId, payload.external_id, payload.email, payload.name,
        payload.avatar, payload.is_active, payload.last_synced_at,
        payload.external_data
      ]);
    }
    ...
    if (userData.external_data.roles && userData.external_data.roles.length > 0) {
...
export async function batchSyncUsers(usersData, provider) {
  ...
  for (const userData of usersData) {
    try {
      ...
      console.error(`Failed to sync user ${userData.id || userData.email}:`, error);
      errors++;
      errorDetails.push({
