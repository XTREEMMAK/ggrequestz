...
export function validateRequiredFields(config, requiredFields, contextName) {
  const cacheKey = createCacheKey(`required:${contextName}`, { config, requiredFields });
...
export function validateOidcConfig(config) {
  ...
  if (!isValidUrl(config.issuer, { requireHttps: process.env.NODE_ENV === 'production' })) {
    return { 
      valid: false, 
...
export function validateApiConfig(config) {
  ...
  if (!isValidUrl(config.baseUrl, { requireHttps: process.env.NODE_ENV === 'production' })) {
    return { 
      valid: false, 
